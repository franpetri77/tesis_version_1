...
//   1. Topbar (no sticky): ubicación + horario + acceso rápido (solo auth o info)
//   2. Main header (sticky): logo + búsqueda + usuario + carrito
//   3. Nav strip (sticky): categorías de acceso directo
//
// Cambios vs versión anterior:
//   - El topbar ya NO duplica el botón "Ingresar" (evita doble navbar)
//   - Para visitantes el topbar solo muestra ubicación/horario
//   - Para autenticados el topbar muestra saludo + pedidos + salir
//   - Alturas reducidas: main h-14 (era h-16), nav strip h-9 (era h-10)
// =============================================

import Link from "next/link";
import {
  ShoppingCart, User, Search, Menu, X, Zap,
  MapPin, LogOut, ShoppingBag, Clock,
} from "lucide-react";
...
// ── Categorías para el nav strip ──
// Los slugs deben coincidir exactamente con los registrados en la tabla `categories` de la DB.
// Slugs actuales: televisores | smartphones | laptops-pcs | audio-video |
//                 linea-blanca | pequenos-electro | gaming | accesorios
const NAV_LINKS = [
...
export function Header() {
  ...
  return (
    <>
      {/* ════════════════════════════════════════════
          TOPBAR — ubicación + horario (no sticky)
          Para visitantes: solo info contextual (sin duplicar auth del main header)
          Para autenticados: saludo + accesos rápidos
      ════════════════════════════════════════════ */}
          ...
          <div className="flex items-center justify-between h-8 text-[11px]">
            ...
            <div className="hidden sm:flex items-center gap-3 text-slate-500">
              <span className="flex items-center gap-1">
                ...
                <span className="font-medium">Córdoba, Alta Gracia</span>
              </span>
            ...
            {/* Derecha: solo para autenticados — visitantes usan el main header */}
            <div className="flex items-center gap-2.5 ml-auto text-slate-400">
              {isAuthenticated && user ? (
                <>
                  <span className="hidden sm:inline text-slate-500">
                    Hola,{" "}
                    <span className="text-slate-200 font-semibold">{user.first_name}</span>
                  </span>
                ...
                /* Visitante: solo texto informativo, sin duplicar botón de ingreso */
                <span className="hidden md:block text-slate-600 text-[10.5px]">
      ...
      <header className="bg-white sticky top-0 z-50 shadow-header">
        ...
        {/* ── Fila principal (h-14 = 56px, más compacto que el anterior h-16) ── */}
        <div className="container-main">
          <div className="flex items-center gap-3 h-14">
            ...
            {/* Acciones — desktop (usuario + carrito) */}
            <div className="hidden md:flex items-center gap-1.5 flex-shrink-0">
              ...
              {isAuthenticated && user ? (
                ...
                >
                  ...
                  <span className="max-w-[72px] truncate">{user.first_name}</span>
                </Link>
              ) : (
                <Link
                  href="/login"
        ...
        {/* ── NAV STRIP — Categorías (solo desktop, h-9 = 36px) ── */}
